...
order1Dim : ℕ
order1Dim = embedDim                -- 2

order2Dim : ℕ
order2Dim = embedDim ℕ* embedDim    -- 4

stateDim : ℕ
stateDim = order0Dim + order1Dim + order2Dim   -- 7

-- Parameter counts (d=2):
--   E:      alphaSize * embedDim        = 54   (embedding matrix)
--   W_state: alphaSize * stateDim       = 189  (state → logits)
--   W_prev: alphaSize * embedDim        = 54   (prevEmb → logits)
...
main : Main
main = run do
  putStrLn "=== Tensor Algebra Predictor ==="
  putStrLn "(Novel architecture from truncated tensor algebra T₂(ℝ^d))"
  putStrLn ""
